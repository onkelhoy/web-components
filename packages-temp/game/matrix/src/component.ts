import { Vector, Vector3, VectorValue } from "@papit/game-vector";
...
export class MatrixN extends Matrix {
  public handedness: "left" | "right" = "right";
  ...
  public rotate(angle: number, axis: number[] = []): this {
    
    if (this.size < 2) throw new Error("Minimum dimension required is 2");

    const hand = this.handedness === "right" ? 1 : -1;
...
export class Matrix4 extends MatrixN {
  ...
  public rotateX(angle: number) {
    return this.multiply(Matrix4.rotationX(angle, this.handedness));
  }

  public rotateY(angle: number) {
    return this.multiply(Matrix4.rotationY(angle, this.handedness));
  }

  public rotateZ(angle: number) {
    return this.multiply(Matrix4.rotationZ(angle, this.handedness));
  }
