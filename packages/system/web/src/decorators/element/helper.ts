import { Setting, Constructor, CustomElement, RenderType } from "./types";

export function combinedStyle(constructor: Constructor) {
  let styles: string[] = [];
  if (constructor.styles) styles = [...constructor.styles];
  if (constructor.style) styles.push(constructor.style);
...
// RENDER FUNCTIONS
export function renderStyle(setting: Setting, element: CustomElement, constructor: Constructor) {
  if (!element.shadowRoot) {
    console.error('shadowroot not defined yet!')
    return;
  }

  // TODO if setting reactivestyles is false we should skip the whole case of render for style 
  ...
  if (!init && !setting.reactiveStyling) {
    return;
  }

  const styles = combinedStyle(constructor);

  if (styles !== element.stylecomperator) {
    styleElement.innerHTML = styles;
    element.stylecomperator = styles;
  }
}
export function renderHTML(setting: Setting, element: CustomElement, constructor: Constructor) {
  if (!element.shadowRoot) {
    console.error('shadowroot not defined yet!')
    return;
  }
  ...
  if (!element.rendertemplate) {
    element.rendertemplate = document.createElement("template");
    // document.create
...
function diffing(element: CustomElement, content: RenderType) {
  if (!element.rendertemplate) return;

  // flush the html
  const nodes = element.rendertemplate.querySelectorAll('*:not(style)');
  for (let node of nodes) {
    node.parentNode?.removeChild(node);
  }
  freshRender(element, content, element.rendertemplate);
...
function cleanup(element: CustomElement) {
  ...
  element.shadowRoot.querySelectorAll('*:not(style)').forEach(node => {
    ...
    const templateNode = (element.rendertemplate as HTMLTemplateElement).querySelector(path.join(" > "));
    if (!templateNode) {
...
function clone(element: CustomElement) {
  ...
  const clone = element.rendertemplate.cloneNode(true) as HTMLTemplateElement;
